...
const ButtonChat = () => {
  ...
  return (
		<div className="fixed bottom-10 z-30 animate-button-chat" style={{
			...
			<AnimatePresence>
				{
					show && (
						<motion.div className="h-full w-full" initial={{ transform: 'translateX(20px)', opacity: 0 }} animate={{ transform: 'translateX(0)', opacity: 1 }} exit={{ transform: 'translateX(20px)', y: 0, opacity: 0 }}>
							<ChatBox/>
						</motion.div>
					)
				}
			</AnimatePresence>
			<button onClick={handleToggle} className={Class('inline-flex absolute bottom-10 right-5 items-center justify-center hover:bg-theme-400 transition-colors duration-200 group hover:text-white h-16 w-16 rounded-full', show ? 'bg-theme-400 text-white' : 'bg-theme-50 text-theme-400')}>
